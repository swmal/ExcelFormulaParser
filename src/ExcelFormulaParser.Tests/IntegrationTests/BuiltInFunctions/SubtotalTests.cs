    ...
    public class SubtotalTests : FormulaParserTestBase
    {
        ...
        public void SubtotalShouldNotIncludeSubtotalChildren()
        {
            _excelDataProvider
                ...
                .Return(new List<ExcelDataItem> { new ExcelDataItem("=SUBTOTAL(9, A2:A3)", 0, 0)});
            _excelDataProvider
                .Stub(x => x.GetRangeValues("A2:A3"))
                .Return(new List<ExcelDataItem> { new ExcelDataItem("=SUBTOTAL(9, A5:A6)", 0, 1), new ExcelDataItem(2d, 0,2)});
            _excelDataProvider
                .Stub(x => x.GetRangeValues("A5:A6"))
                .Return(new List<ExcelDataItem> { new ExcelDataItem(2d, 0, 4), new ExcelDataItem(2d, 0,5)});
